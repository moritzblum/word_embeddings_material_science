...
import chemdataextractor.errors
import numpy as np
import argparse
...
def tsne_plot(model):
    ...
    labels = [label for label in labels if model.wv.get_vecattr(label, "count") < args.max]
...
if __name__ == '__main__':
    ...
    if args.source != '':
        print('Reads PDFs and outputs tokens to tokenized.json.')
        pdf_dir = args.source
        sentences = []
        files = list(filter(lambda x: not x.startswith('.') and x.endswith('.pdf'), listdir(pdf_dir)))
        for file in files:
            ...
            except chemdataextractor.errors.ReaderError:
